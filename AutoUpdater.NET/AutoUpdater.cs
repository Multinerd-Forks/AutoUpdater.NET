...
namespace AutoUpdaterDotNET
{
    ...
    public static class AutoUpdater
	{
        ...
        public static void Start(Assembly myAssembly = null) { Start(AppCastURL, myAssembly); }
				...
				else
				{
					...
					else
					{
						if (args != null)
						{
							...
							{
								if (!IsWinFormsApplication) Application.EnableVisualStyles();
		...
		private static void BackgroundWorkerDoWork(object sender, DoWorkEventArgs e)
		{
			...
			try
			{
				...
				else if (uri.Scheme.Equals(Uri.UriSchemeHttp) || uri.Scheme.Equals(Uri.UriSchemeHttps))
				{
					...
					if (BasicAuthXML != null) httpWebRequest.Headers[HttpRequestHeader.Authorization] = BasicAuthXML.ToString();
			...
			using (var appCastStream = webResponse.GetResponseStream())
			{
				if (appCastStream != null)
				{
					...
					{
						var receivedAppCastDocument = new XmlDocument {XmlResolver = null};
			...
			{
				webResponse.Close();
				if (ReportErrors) throw new InvalidDataException();
			...
			else
			{
				...
				foreach (var process in Process.GetProcessesByName(currentProcess.ProcessName))
				{
					...
					) //get all instances of assembly except current
					{
						...
						if (!process.HasExited) process.Kill(); //TODO show UI message asking user to close program himself instead of silently killing it
					}
				}
		...
		internal static string GetUserAgent() { return string.IsNullOrEmpty(HttpUserAgent) ? "AutoUpdater.NET" : HttpUserAgent; }
