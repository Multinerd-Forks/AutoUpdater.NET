...
namespace AutoUpdater.NETStandard
{
	public class Updater
	{
        ...
        /// </summary>
        /// <param name="args">
        ///     An object containing all the parameters received from XML file. If there will be an error while
        ///     looking for the XML file then this object will be null.
        /// </param>
				...
				using (var response = await client.GetAsync(url))
				{
					using (var content = response.Content)
					{
						if (ParseUpdateInfo == null)
						{
							...
							var jsonString = JsonConvert.SerializeXmlNode(doc.LastChild, Formatting.Indented, true);
#else
                    var jsonString = await response.Content.ReadAsStringAsync();
#endif
							updateInfo = JsonConvert.DeserializeObject<UpdateInfo>(jsonString);
						}
						else
						{
							var parseArgs = new ParseUpdateInfoEventArgs(await content.ReadAsStringAsync());
		...
		public void Download(UpdateInfo updateInfo)
		{
			...
			if (Proxy != null) _webClient.Proxy = Proxy;
			...
			if (BasicAuthDownload != null) _webClient.Headers[HttpRequestHeader.Authorization] = BasicAuthDownload.ToString();

			_webClient.DownloadProgressChanged += (sender, args) =>
			{
				var downloadProgressEventArgs = new DownloadProgressEventArgs();
			...
			_webClient.DownloadFileCompleted += (sender, args) =>
			{
				...
				try
				{
					...
					if (updateInfo.CheckSum != null) CompareChecksum(tempFile, updateInfo.CheckSum);
		...
		{
			if (_webClient != null && _webClient.IsBusy) _webClient.CancelAsync();
		}
        ...
        private HttpClient GetClient()
        {
            ...
            if (BasicAuthXML != null)
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic", BasicAuthXML.ToString());
